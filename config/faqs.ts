...
export const FAQS_ZH = [
  {
    title: "这款应用是如何保护用户隐私的?",
    content:
      "我们的应用采用高级端到端加密技术,确保所有通信和位置数据的安全性。用户可以灵活设置位置共享时间段,随时控制隐私。我们不会存储或出售用户的个人数据,您的隐私安全是我们的首要考虑。",
  },
  {
    title: "这款应用会不会很耗电?",
    content:
      "我们的应用经过优化,以最小化电池消耗。尽管需要持续获取位置信息,但我们采用了智能算法来平衡定位精度和电量消耗。大多数用户反馈,使用我们的应用并没有明显增加手机耗电量。",
  },
  {
    title: "如何设置位置共享?",
    content:
      "设置位置共享非常简单。首先,在应用中邀请您的伴侣。一旦对方接受邀请,您就可以在设置中选择'位置共享'选项,然后选择共享的时间段(例如,全天候或特定时间)。您随时可以在设置中修改或暂停位置共享。",
  },
  {
    title: "应用支持哪些运动状态追踪?",
    content:
      "我们的应用可以识别多种运动状态,包括步行、跑步、骑行、驾驶等。系统会自动检测用户的运动状态,并实时更新。这项功能可以让您的伴侣了解您当前的活动情况。",
  },
  {
    title: "如何查看伴侣的运动轨迹?",
    content:
      "在主界面的地图上,您可以看到一个'轨迹'按钮。点击它,然后选择您想查看的时间段(例如,今天、昨天或过去一周),系统就会在地图上显示您伴侣的运动轨迹。这个功能让您能够了解伴侣的日常活动模式。",
  },
  {
    title: "应用是否支持跨平台使用?",
    content:
      "是的,我们的应用支持iOS和Android平台。无论您和您的伴侣使用什么类型的智能手机,都可以顺畅地使用我们的应用。我们确保在不同平台上提供一致的用户体验。",
  },
  {
    title: "如果我想暂时关闭位置共享,该怎么做?",
    content:
      "在应用的设置中,有一个'隐私模式'选项。启动这个模式后,您的位置将不会被共享,直到您再次关闭隐私模式。这个功能让您能够在需要时轻松控制自己的隐私。",
  },
  {
    title: "应用是否提供紧急求助功能?",
    content:
      "是的,我们的应用内置了紧急求助功能。在主界面有一个'SOS'按钮,点击后会立即向您预设的紧急联系人(可以是您的伴侣或其他人)发送您的实时位置和求助信息。这个功能旨在为用户提供额外的安全保障。",
  },
  {
    title: "能否设置自定义的位置提醒?",
    content:
      "当然可以。在应用中,您可以设置地理围栏。当您的伴侣进入或离开某个特定区域时(例如家或工作地点),您会收到通知。这个功能可以帮助您更好地协调日常生活。",
  },
  {
    title: "应用是否收费?有哪些付费功能?",
    content:
      "我们提供基础版和高级版。基础版免费,包含核心的位置共享和通讯功能。高级版则提供更多高级功能,如详细的运动分析、更长期的位置历史记录等。您可以在应用内查看具体的价格和功能对比。",
  },
];
